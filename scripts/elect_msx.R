...
# It contains the code for Figure 4 of the manuscript
...
##### Combo DAFM/TimeOfDay manuscript figure #####
# This is the code for Figure 4 of the manuscript
# Code copied from above
dafm_p <- ggline(freqDAFM_plot, x="DAFM",y="value",color = "type",
       plot_type = "l", xlab="Days after full moon (DAFM)",
       ylab="Chesson Election Index") +
   scale_color_manual(name="", 
                      values=c(ChessonS="mediumaquamarine",
                               ChessonS_abs="orangered1",
...
freqHalfHourEI <- freqHalfHourEI %>% 
   mutate(mssca=factor(mssca,levels=c("out","in")))
halfhour_p <- ggplot() +
   # Highlight spawning time
   geom_vline(aes(xintercept=spawnlim[1]), linetype="dotted") +
   geom_vline(aes(xintercept=spawnlim[2]), linetype="dotted") +
...
chesson_figure <- ggarrange(dafm_p, halfhour_p, labels = c("A","B"), ncol=1)
# ggexport(chesson_figure, filename = "outputs/chesson_figure_20231201.jpeg",
#          width = 900, height = 1000)
# ggexport(chesson_figure, filename = "outputs/Fig4_chesson_202404.tiff", res=300,
